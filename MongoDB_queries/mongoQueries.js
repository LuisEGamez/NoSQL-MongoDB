...
//15. Escriu una consulta per trobar el restaurant_id, name, borough i cuisine per a aquells restaurants quin contenir 'ces' com les últimes tres lletres en el seu nom
...
//17. Escriu una consulta per trobar els restaurants quins pertanyen al Bronx i va preparar qualsevol plat American o xinés. "American " va con un espacio al final
...
db.restaurantes.find({"grades.score": {$lte: 10}}, {restaurant_id:1, name: 1, borough:1, cuisine:1})
...
db.restaurantes.find({$nor: [{cuisine: "American "}, {borough: "Chinese"}, {name: {$regex: /^Will/}}]}, {restaurant_id:1, name: 1, borough:1, cuisine:1})

// No entiendo a que se refiere con que preparen peces no encuentro ningun campo con ese valor
...
db.restaurantes.find({grades: {$elemMatch: {"date": ISODate ("2014-08-11T00:00:00Z"), "grade": "A", "score": 11 }}}, {restaurant_id:1, name: 1, grades:1})
...
db.restaurantes.find({"grades.1": {$elemMatch: {"date": ISODate ("2014-08-11T00:00:00Z"), "grade": "A", "score": 9 }}}, {restaurant_id:1, name: 1, grades:1})

//24. Escriu una consulta per trobar el restaurant_id, name, adre a i ubicaci  geogr fica per a aquells restaurants on el segon element del array coord conte un valor quin es mes que 42 i fins a 52
...
db.restaurantes.find({"address.street": {$exists: false}},{_id:0,restaurant_id:1 ,name:1, "address.street":1})
...
db.restaurantes.find({"address.coord": {$type: "double"}}, {_id:0,restaurant_id:1 ,name:1, "address.coord":1})
...
db.restaurantes.find({name: {$regex:/mon/}}, {_id:0, name:1, borough:1, "address.coord":1, cuisine:1}) // Case-sensitive

db.restaurantes.find({name: {$regex:/mon/i}}, {_id:0, name:1, borough:1, "address.coord":1, cuisine:1})// Case-insensitive
...
db.restaurantes.find({name: {$regex:/^Mad/}}, {_id:0, name:1, borough:1, "address.coord":1, cuisine:1}) // Case-sensitive
